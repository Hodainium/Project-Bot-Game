...
#include "PBGameplayAbility_RangedWeapon.generated.h"
...
UENUM(BlueprintType)
enum class EPBAbilityTargetingSource : uint8
{
...
/**
 * UPBGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UPBGameplayAbility_RangedWeapon : public UPBGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UPBGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "PB|Ability")
	UPBRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FPBRangedWeaponFiringInput
	{
		...
		UPBRangedWeaponInstance* WeaponData = nullptr;

		// Can we play bullet FX for hits during this trace
		bool bCanPlayBulletFX = false;

		FPBRangedWeaponFiringInput()
...
protected:
	...
	void TraceBulletsInCartridge(const FPBRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits);
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EPBAbilityTargetingSource Source) const;
