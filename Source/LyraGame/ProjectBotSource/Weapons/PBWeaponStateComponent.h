...
#include "PBWeaponStateComponent.generated.h"
...
struct FPBScreenSpaceHitLocation
{
...
struct FPBServerSideHitMarkerBatch
{
	FPBServerSideHitMarkerBatch() { }

	FPBServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FPBScreenSpaceHitLocation> Markers;
...
UCLASS()
class UPBWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UPBWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FPBScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FPBScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FPBServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
