...
#include "PBWeaponStateComponent.h"
...
UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_PB_Gameplay_Zone, "Gameplay.Zone");

UPBWeaponStateComponent::UPBWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UPBWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UPBWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool UPBWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UPBWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FPBServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FPBScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UPBWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FPBServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FPBScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
				...
				if (const UPhysicalMaterialWithTags* PhysMatWithTags = Cast<const UPhysicalMaterialWithTags>(Hit.PhysMaterial.Get()))
				{
					...
					{
						if (MaterialTag.MatchesTag(TAG_PB_Gameplay_Zone))
						{
...
void UPBWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UPBWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UPBWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
